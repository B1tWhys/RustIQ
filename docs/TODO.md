...
- [ ] Delete `engine/sdr.rs` entirely (using RustRadio's source blocks instead)

### 4.2 Implement SpectrumSink
- [ ] Create `engine/sinks/` directory structure
- [ ] Implement `SpectrumSink` in `engine/sinks/spectrum.rs`
...
- [ ] Write test: construct `Engine::new(cmd_rx, event_tx)` (will initially fail to compile)

### 4.4 Implement Engine Public API
- [ ] Implement `Engine` struct in `engine/mod.rs` (holds Graph, cmd_rx)
- [ ] Implement `Engine::new(cmd_rx, event_tx)` constructor
  - Call `graph::build_graph(event_tx)` to create graph
- [ ] Test from 4.3 now compiles

### 4.5 Test: Engine Runs
- [ ] Update test to call `engine.run()` in spawned thread
- [ ] Test should compile but will fail at runtime (no graph implementation yet)

### 4.6 Implement Basic Graph (No FFT)
- [ ] Create `engine/graph.rs`
- [ ] Implement `build_graph(event_tx)` function
...
- [ ] Implement `Engine::run()` method in `engine/mod.rs`
  - Send `StateSnapshot` on startup via event_tx
  - Call `self.graph.run()`
- [ ] Test from 4.5 now passes (receives events)

### 4.8 Test: Verify StateSnapshot
- [ ] Write test: verify first event is `StateSnapshot`
- [ ] Verify `EngineState` contains correct sample_rate, center_frequency

### 4.9 Test: Verify SpectrumData Events
- [ ] Write test: verify `SpectrumData` events are received
- [ ] Test receives multiple events successfully
